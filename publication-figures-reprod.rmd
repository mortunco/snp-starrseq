...
library(tidyverse)
library(vroom)
mylist=list.files("data",pattern = "cluster",full.names = T)
...
ggplot(temp,aes(n,fill=sample)) + 
 ...
 ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/asym-longshort-shortlong-cluster-coverage.pdf",width = 6,height = 4) +
 NULL
...
ggplot(temp %>% filter(n>1),aes(n,fill=sample)) + 
 ...
 theme_bw()+
 theme(legend.position = "top",text=element_text(size=18)) + 
 #ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/asym-longshort-shortlong-cluster-coverage-1-removed.pdf",width = 6,height = 4)+
 NULL 
...
ggplot(temp %>% filter(n>1),aes(sample,n,fill=sample)) + 
 ...
 theme_bw()+
 theme(legend.position = "top",text=element_text(size=18)) +
 ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/asym-boxplot-longshort-shortlong-cluster-coverage.pdf",width = 4,height = 4)+
 NULL 
...
#ggsave(plot=p,filename="pathogenecity-dotplot-deltaSVM-padj.pdf",width = 8,height = 8)
```
...
snplist=vroom("308.bed",col_names=F)
loop_gene=vroom("hichip-gene.txt",col_names=F) %>% 
    select(X4,X5,X9) %>% dplyr::rename(anchorID=X4, loopscore=X5, gene=X9)
loop_snp=vroom("hichip-snp.txt",col_names=F) %>% 
    select(X4,X9) %>% 
...
nom_sig=vroom("sig_event.txt",col_names=F) %>% pull(X4)
loop_snp= loop_snp %>% mutate(sig= X9 %in% nom_sig) 

claudia_genes=vroom("claudia-eqtl-genes.txt",col_names=F,delim="\t") %>% pull(X1)
...
h24=vroom('data/raw_results_merged_24h.txt',delim = "\t",show_col_types = FALSE,id = "sample")  %>% mutate(sample=gsub(pattern ="raw_results_merged_",replacement = "",x = basename(sample)))
h24 = h24 %>% filter(alt_count >=15 & ref_count >=15) 
h24$padj=p.adjust(h24$pvalue,method = "fdr",n = nrow(h24))

h48=vroom('data/raw_results_merged_48h.txt',delim = "\t",show_col_types = FALSE,id = "sample")  %>% mutate(sample=gsub(pattern ="raw_results_merged_",replacement = "",x = basename(sample)))
h48 = h48 %>%  filter(alt_count >=15 & ref_count >=15)
h48$padj=p.adjust(h48$pvalue,method = "fdr",n = nrow(h48))

h72=vroom('data/raw_results_merged_72h.txt',delim = "\t",show_col_types = FALSE,id = "sample")  %>% mutate(sample=gsub(pattern ="raw_results_merged_",replacement = "",x = basename(sample)))
h72 = h72 %>% filter(alt_count >=15 & ref_count >=15)
h72$padj=p.adjust(h72$pvalue,method = "fdr",n = nrow(h72))
