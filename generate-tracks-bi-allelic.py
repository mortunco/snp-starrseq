...
parser.add_argument("-i","--vis_file",type=str,required=True,help="vis info file.")
...
parser.add_argument("-o","--output-dir",type=str,help="output directory that contains all bedgraph files.")

def generate_bedgraph(index_event, event_type, snpid,chromsizes,event):
...
index_event=index_event.merge(vis_info,left_on="start_aln_UMI",right_on="index",validate="one_to_many",how="inner")
...
for event in result_table["event"].to_list():
    ...
    print(f'creating bedgraph for {event} {snpid}')
    generate_bedgraph(index_event, 1, snpid, chromsizes, event)
    print("WT done!")
    generate_bedgraph(index_event, 2, snpid, chromsizes, event)
    print("VAR done!")
