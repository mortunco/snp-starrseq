...
for barcode,clusters in barcode_to_cid.items():
  if len(clusters) == 1:
    ...
    if (check_names_same == [False,False]) or (check_names_same == [True,True]):
      ...
      clustered_r1.write("@"+barcode+"\n")
      clustered_r1.write(max(cid_to_readinfo[clusters[0]]["forward"]["seq"],cid_to_readinfo[clusters[1]]["forward"]["seq"],key=len) + "\n")
      clustered_r1.write("+\n")
      clustered_r1.write(max(cid_to_readinfo[clusters[0]]["forward"]["qual"],cid_to_readinfo[clusters[1]]["forward"]["qual"],key=len) + "\n")
      clustered_r1.write("\n")
      clustered_r2.write("@"+barcode+"\n")
      clustered_r2.write(max(cid_to_readinfo[clusters[0]]["reverse"]["seq"],cid_to_readinfo[clusters[1]]["reverse"]["seq"],key=len) + "\n")
      clustered_r2.write("+\n")
      clustered_r2.write(max(cid_to_readinfo[clusters[0]]["reverse"]["qual"],cid_to_readinfo[clusters[1]]["reverse"]["qual"],key=len) + "\n")
      clustered_r2.write("\n")
  else:
    #print(clusters)
    #print(len(clusters))
