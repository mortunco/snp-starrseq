...
mydf=vroom(mylist,id = "sample",delim = "\t",col_names = c("cid","nid","rrow","r1n","r1s","r1q","r2n","r2s","r2q")) %>% select(cid,nid,rrow,sample) %>% mutate(sample=gsub(pattern = ".cluster",replacement = "",x = basename(sample))) %>% as_tibble

mydf %>% as_tibble %>% group_by(sample,cid) %>% dplyr::count() -> temp

### basic stats ###
temp %>% ungroup %>% filter(sample =="longshort" & n > 1) %>% select(n) %>% apply(.,2,summary)
temp %>% ungroup %>% filter(sample =="longshort" & n > 1) %>% pull(n)-> temp_x
...
temp %>% ungroup %>% filter(sample =="shortlong" & n > 1) %>% pull(n)-> temp_x
sprintf("Standart Error Of mean is %s", sd(temp_x)/sqrt(length(temp_x)))
...
ggplot(temp,aes(n,fill=sample)) + 
 ...
 NULL + ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/asym-longshort-shortlong-cluster-coverage.pdf",width = 6,height = 4)
...
ggplot(temp %>% filter(n>1),aes(n,fill=sample)) + 
 ...
 theme_bw()+
 theme(legend.position = "top",text=element_text(size=18)) + 
 NULL + ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/asym-longshort-shortlong-cluster-coverage-1-removed.pdf",width = 6,height = 4)
...
ggplot(temp %>% filter(n>1),aes(sample,n,fill=sample)) + 
 ...
 theme_bw()+
 theme(legend.position = "top",text=element_text(size=18)) +
 NULL +  ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/asym-boxplot-longshort-shortlong-cluster-coverage.pdf",width = 4,height = 4)
...
library(tidyverse)
library(reshape2)
h48=vroom('/groups/lackgrp/projects/dev-snpstarrseq-pipeline/yi-results-48/yi-merged-result-final/raw_results_merged_48h.txt',delim = "\t",show_col_types = FALSE,id = "sample")  %>% mutate(sample=gsub(pattern ="raw_results_merged_",replacement = "",x = basename(sample))) %>% filter(alt_count >=15 & ref_count >=15)
h48$padj=p.adjust(h48$pvalue,method = "fdr",n = nrow(h48))

cadd_res = vroom("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis//publication-figures/pahthogenecity-testing/cadd-prediction/GRCh37-v1.6_6123ca426476f4eb96fdfaa8519c34fd.tsv",delim = "\t",show_col_types = F) %>% rowwise %>% mutate(event=sprintf("chr%s;%s;%s;%s",Chrom,Pos,Ref,Alt))
...
p<- mydf %>% 
    ...
    ylab("CADD Score ~ PHRED")+
    NULL
ggsave(plot=p,"/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis//publication-figures/pathogenecity-dotplot-CADD-padj.pdf",width = 8,height = 8)  
```
### ncER
```{r}
...
ncER = vroom("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis//publication-figures/pahthogenecity-testing/ncER/ncER-overlap",delim = "\t",show_col_types = F,col_names = c("c","s","e","ref","alt","nc_c","nc_s","nc_e","nc_score")) %>% 
...
mydf %>% 
    ...
    ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/pathogenecity-boxplot-ncER.pdf",width = 6,height = 6)+
    NULL
...
mydf %>% 
    ...
    ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/pathogenecity-dotplot-ncER-alt_effect.pdf",width = 8,height = 8)+
    NULL
...
p<-mydf %>% 
    ...
    ylab("ncER ~ Essentiality Score")+
    NULL
ggsave(plot=p,filename="/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/pathogenecity-dotplot-ncER-padj.pdf",width = 8,height = 8)
```
### DeltaSVM 
```{r}
...
mydf %>% 
    ...
    xlab("48hr Alt Effect ~ log2fc(ALT/WT)")+
    ggsave("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures//pathogenecity-boxplot-deltaSVM-alteffect.pdf",width = 6,height = 6)+
    NULL
...
p<-mydf %>% 
    ...
    theme_bw()+
    theme(text=element_text(size = 25),legend.position = "top") +
    xlab("48hr Event Significance ~ -log10(pvalue)")+
    ylab("DeltaSVM ~ Essentiality Score")+
    #ggsave("../analysis/publication-figures/pathogenecity-dotplot-deltaSVM-padj.pdf",width = 8,height = 8)+
    NULL

ggsave(plot=p,filename="/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/pathogenecity-dotplot-deltaSVM-padj.pdf",width = 8,height = 8)
```
...
### paper data, remove this 
mydf %>%
  filter(RSID %in% common_events_nom) %>%
  reshape2::dcast(RSID~sample, value.var = "af",fun.aggregate = NULL) %>%
  rowwise() %>%  filter(!is.na(st) & h!all(c(is.na(ar),is.na(hk),is.na(atac)))) %>%
  data.table::melt(., id.vars=c("RSID","st")) ->  temp
temp %>% filter(!is.na(value)) %>% count(RSID) %>% pull(n) %>% table()
#####
...
h24=vroom('/groups/lackgrp/projects/dev-snpstarrseq-pipeline/yi-results-48/yi-merged-result-final/raw_results_merged_24h.txt',delim = "\t",show_col_types = FALSE,id = "sample")  %>% mutate(sample=gsub(pattern ="raw_results_",replacement = "",x = basename(sample)))
...
h72=vroom('/groups/lackgrp/projects/dev-snpstarrseq-pipeline/yi-results-48/yi-merged-result-final/raw_results_merged_72h.txt',delim = "\t",show_col_types = FALSE,id = "sample")  %>% mutate(sample=gsub(pattern ="raw_results_",replacement = "",x = basename(sample)))
...
snplist=vroom("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/308.bed",col_names=F)
...
sheet_write(data=loop_snp,ss = gs4_create("snpstarrseq-sup-snp-gene"))
```
...
viz_waterfall <- function(plot_df,name=""){
  ...
  left<- ggplot(plot_df %>% filter(type == "snp"),aes(x = index,y = value,fill=type)) +
    ...
    ggsave(sprintf("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/waterfall_normalized_to_lib%shrs.pdf",name),width = 6,height = 6)+
    NULL
}
...
mydf %>% filter(pvalue <= 0.05) %>% pull(event) %>% unique -> common_events_nom
mydf %>% filter(event %in% common_events_nom & pvalue <= 0.05 ) %>% group_by(event) %>% sli ce_max(abs(alt_effect)) %>% arrange(desc(abs(alt_effect))) -> temp

mydf %>% filter(padj <= 0.05) %>% pull(event) %>% unique -> common_events_FDR
mydf %>% filter(event %in% common_events_FDR & padj <= 0.05 ) %>% group_by(event) %>% slice_max(abs(alt_effect)) %>% arrange(desc(abs(alt_effect))) -> temp

library(googlesheets4)
#sheet_write(data=mydf,ss = gs4_create("snpstarrseq-NBR-results-all-times"))
...
bal = vroom("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/full-data-strict/3-generate-fragment-lib/barcode-allele.tsv",delim="\t",num_threads=16) %>% select(-fragment_name)
bal %>% filter(mutation %in% h48$event)  -> bal

### number of unique allele count based on barcode allele
h48 %>% summary
bal %>% group_by(mutation) %>% dplyr::count(type) %>% group_by(type) %>% summarize(x=median(n))

### read coun tables ###
mylist=c("/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/full-data-strict/4-symmetric-barcode-quantification/startposcounts.SNP_CL.txt",
"/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/full-data-strict/4-symmetric-barcode-quantification/startposcounts.SNPR1_48.txt",
"/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/full-data-strict/4-symmetric-barcode-quantification/startposcounts.SNPR2_48.txt",
"/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/full-data-strict/4-symmetric-barcode-quantification/startposcounts.SNPR3_48.txt")
count_table = vroom(mylist,delim=" ",num_threads=50,id="sample",col_names=c("count","index"))
count_table$sample = basename(count_table$sample)
### get overlapping counts ###
count_table %>% filter(index %in% bal$start_aln_UMI) -> count_table
count_table %>% reshape2::dcast(., formula= index ~ sample, value.var="count",fill=0) -> count_table

### ssumm 48_1 48_2 48_3
count_table %>% rowwise() %>% mutate(sum_x=`startposcounts.SNPR1_48.txt`+`startposcounts.SNPR2_48.txt`+`startposcounts.SNPR3_48.txt`) %>% rename(lib = `startposcounts.SNP_CL.txt` ) %>% select(index,lib,sum_x) ->count_table

count_table=left_join(count_table, bal, by=c("index"= "start_aln_UMI"))

### Total Reads per SNP (WT + VAR summed up) ###
count_table %>% 
    group_by(mutation) %>% 
    summarize(lib=sum(lib),sum_x=sum(sum_x)) %>% summary()

### the plot.
p <- count_table %>% 
    group_by(mutation) %>% 
    summarize(lib=sum(lib),sum_x=sum(sum_x)) %>% 
    ggplot(.,aes(log2(sum_x))) + 
        geom_histogram() + 
        scale_x_continuous(breaks=scales::pretty_breaks(8)) +
        theme_bw(base_size=20)+
        theme(legend.position = "none",panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
        xlab("Total Reads per SNP(WT+VAR) across 308 SNPs")+
        ylab("Count")+
        NULL
ggsave(plot=p,filename="/groups/lackgrp/projects/dev-snpstarrseq-pipeline/analysis/publication-figures/SNP_plasmid_read_coverage.pdf",height = 4,width = 8)
```
